...
class SyncEngine {
  ...
  Future<Result<void>> forceFullSync({required int studentId}) async {
    if (_state.isSyncing) return const Result.failure(UnknownFailure(message: 'Sync in progress'));
  ...
  Duration get nextSyncInterval => _backoff.intervalAfterFailures(_consecutiveFailures);
